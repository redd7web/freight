...
switch($_GET['mode']){
    case "grease":
        $schedule_table = $dbprefix."_grease_traps";
        $route_id = "grease_route_no";
    break;
}
...
$full = $db->query("SELECT 
    ...
    FROM $schedule_table INNER JOIN $account_table ON $schedule_table.account_no = $account_table.account_ID WHERE (route_status('scheduled') AND $route_id IS NULL ) && (status !='archive') && $account_table.division = $person->facility && $schedule_table.code_red=1");    
