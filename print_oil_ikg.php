...
$html .='<div id="fields" style="width: 100%;height:auto;margin:auto;">
    ...
    <table style="width:100%;margin:auto;height:125px;font-size:14px;" id="chart">
    <thead id="meathead">
    <tr>        
        <td style="padding: 0px 0px 0px 0px;width:9px;height:100px;">#</td>
        <td style="padding: 0px 0px 0px 0px;width:200px;">Name</td>
        
        <td  style="padding: 0px 0px 0px 0px;width:200px;">Address</td>
        <td style="padding: 0px 0px 0px 0px;width:150px;">City</td>
        <td style="padding: 0px 0px 0px 0px;width:50px;">Zip</td>
        <td style="padding: 0px 0px 0px 0px;width:60px;">Tote Size</td>
        <td style="padding: 0px 0px 0px 0px;width:20px;">Inches</td>
        <td style="padding: 0px 0px 0px 0px;width:25px;">Gals</td>
        <td style="padding: 0px 0px 0px 0px;width:70px;width:80px;">Report</td>
        <td style="padding: 0px 0px 0px 0px;width:70px;width:80px;">Notes</td>
        <td style="padding: 0px 0px 0px 0px;width:70px;width:80px;">Lb/Mile</td>
        <td style="padding: 0px 0px 0px 0px;width:25px;">Net</td>
    </tr>
    </thead>
    <tbody id="meat">
';
    ...
    $count =1;
         foreach ($ikg_info->scheduled_routes as $ekc ){  
            ...
            $html .= "<tr>";
                $html .= "<td style='height:50px;'>$count</td>";
                $html .= "<td>".trim($sched_ro->account_name)."</td>";
                $html .= "<td>".trim($aaccoouunntt->singleField($sched_ro->account_number,"address"))."</td>";
                $html .= "<td>".trim($aaccoouunntt->singleField($sched_ro->account_number,"city"))."</td>";  
                $html .= "<td>".trim($aaccoouunntt->singleField($sched_ro->account_number,"zip"))."</td>";   
                $html .= "<td>";  
                    ...
                    if(count($get)>0){
                         foreach($get as $info){
                            $html .= trim($info['amount_holds'])."</br>";
                         }    
                    }
                $html .="</td>";
                $html .= "<td> "; 
                if(count($request_inches)>0){
                    foreach($request_inches as $entered){
                        $html .= trim($entered['inches_entered'])."<br/>";
                    }
                }
                else {
                    $html .= "";
                ...
                $html .= "<td>";
                    if(count($request_inches)>0){
                       foreach($request_inches as $inches){
                         $html .= trim(round($inches['inches_to_gallons'],2))."<br/>";
                       }
                    }
                    else {
                        $html .= "";
                ...
                $html .= "</td>";
                 ...
                 if(count($bh)>0){
                    $html .= trim($bh[0]['fieldreport']);
                 } else {
                     $html .="";
                 } 
                 $html .="</td>"; 
                 
                 if(strlen($aaccoouunntt->singleField($sched_ro->account_number,"notes"))>0){
                    $html .="<td>".trim($aaccoouunntt->singleField($sched_ro->account_number,"notes"))."</td>";//static notes   
                 } else {
                    $html .="<td></td>";//static notes
                 }
                  
                 
                 $html .="<td>";//net mileage
                    if($sched_ro->route_id !=null){
                        $io = $db->query("SELECT DISTINCT(schedule_id),inches_to_gallons*7.56 AS net_weight,mileage FROM freight_data_table WHERE schedule_id=$sched_ro->schedule_id AND route_id=$sched_ro->route_id AND mileage IS NOT NULL AND mileage >0");
                        if(count($io)>0){
                            if($preVal == null){
                                $difc = $io[0]['mileage']- $ikg_info->start_mileage;
                                $html .= number_format($io[0]['net_weight']/$difc,2);
                            }else {
                                $difc = $io[0]['mileage'] - $preVal;
                                $html .= number_format($io[0]['net_weight']/$difc,2);
                            }
                            $preVal = $io[0]['mileage'];       
                        }else {
