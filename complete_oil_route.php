...
if(count($req)>0){
    foreach($req as $stops){        
        ...
        echo "UPDATE sludge_scheduled_routes SET route_status='completed' WHERE route_id=$_GET[route_id] AND schedule_id=$stops[schedule_id]<br/>";
        $db->query("UPDATE sludge_scheduled_routes SET route_status='completed' WHERE route_id=$_GET[route_id] AND schedule_id=$stops[schedule_id]");
        ...
        //echo "</pre>";
        $db->insert("sludge_scheduled_routes",$schedule_info)."</br>";
    ...
    $db->query("UPDATE sludge_ikg_manifest_info SET account_numbers='$nums',completed_date='$date' WHERE route_id=$_GET[route_id]");
...
$db->query("UPDATE sludge_ikg_manifest_info SET completed_date='$date' WHERE route_id=$_GET[route_id]");
 $db->query("UPDATE sludge_list_of_routes SET stops = $count, collected = $tot, expected = $avg,status='completed',completed_date='$date' WHERE route_id=$_GET[route_id]");
