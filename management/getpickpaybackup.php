...
function pickups($account_no,$range = null){
    ...
    $info = $db->query("SELECT date_of_pickup FROM $data_table WHERE account_no = $account_no $range ORDER BY date_of_pickup DESC");
    if(count($info)>0){
        //latest|first
...
if(isset($_POST['search_now'])){
    ...
    if($_POST['my_group'] =="-"){ // default search
        $ask = "SELECT distinct(freight_grease_data_table.date_of_pickup),
...
freight_grease_data_table.payment_method,
COUNT( DISTINCT (freight_grease_data_table.date_of_pickup) ) as num_pickups,
freight_accounts.contact_name, 
freight_accounts.status,
freight_accounts.account_ID, 
freight_accounts.address,
