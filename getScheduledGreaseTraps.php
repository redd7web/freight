...
if( isset($_POST['search_now'])  ){ //********************  POSTED SEARCH  ********************************
    ...
    $check = $db->query("SELECT * FROM $grease_table WHERE route_status IN ('scheduled','new') AND grease_route_no IS NULL AND fire =1 $addtn");
} else {
    ...
    if(!empty($_SESSION['temp_stops'])){
        foreach($_SESSION['temp_stops'] as $saved){
            ...
            echo "<td style='vertical-align:top;text-align:left;'>
                <img src='img/delete-icon.jpg' class='delgrease' rel='".$saved_gs->grease_no."' style='width:15px;height:15px;cursor:pointer;float:left;;display:block'/>                
                <input type='checkbox' class='setThisRoute cachedx' style='cursor:pointer;width:10px;height:15px;z-index:9999;float:left;;display:block;margin-left:5px;' checked='true'   xlr='".$saved_gs->account_number."' rel='".$saved_gs->grease_no."' "; 
...
if(count($check)> 0){
    ...
    foreach($check as $grease){
        if(!in_array($grease,$_SESSION['temp_stops'])   ){
...
$act[]="";//ikg_manifest_route_number,route_id
$ac = $db->query("SELECT route_id,ikg_manifest_route_number FROM freight_list_of_grease WHERE status='completed'");
if(count($ac) !=0){   
    foreach($ac as $value){        
        $act[] = "'$value[ikg_manifest_route_number] ~ $value[route_id]'  \r\n";
    }
}
...
$("#all_frack").click(function(){
    if( $(this).is(":checked") ){
        $(".frac").prop("checked",true);
    }else{
        $(".frac").prop("checked",false);
    }
});
